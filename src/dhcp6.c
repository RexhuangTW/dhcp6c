...
#define DST_PORT 547
#define SRC_PORT 546
#define HELLO_GROUP "ff02::1:2"

int add_option(uint8_t *dest, enum option_code opt_code, uint16_t leng, uint8_t *data)
{  
    struct dhcpv6_option *opt_packet = (struct dhcpv6_option *)dest;

    opt_packet->option = htons(opt_code);   
    opt_packet->len = htons(leng);   
    memcpy(opt_packet->value, data, leng);

    return (sizeof(struct dhcpv6_option)+leng);
}
int add_message(uint8_t *dest, enum message_type msg_type, uint16_t leng, uint8_t *data)
{
    struct dhcpv6_soliocit *opt_packet = (struct dhcpv6_soliocit *)dest;

    opt_packet->message_type = msg_type;   
    memcpy(opt_packet->trans_ID, data, leng);

    return (sizeof(struct dhcpv6_soliocit)+leng);	
}
...
int int2hex(uint8_t *dest, int num)
{
	dest[1] = num & 0xFF;
	dest[0] = (num >>8)& 0xFF;
}
...
void main(int argc, char *argv[])
{
	...
	inet_pton(AF_INET6, HELLO_GROUP, &addr.sin6_addr);
	...
	ia_id_v->t1 = htons(0);
	ia_id_v->t2 = htons(0);
	offest += add_option(buf+offest, OPTION_IA_NA, sizeof(struct dhcpv6_IA_ID), ia_id_v);
